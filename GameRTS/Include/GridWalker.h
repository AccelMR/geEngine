...
public:
  ...
  virtual bool weightedGraphSupported() {
    return false;
  }				//Indica si este Walker soporta la asignación y el uso de pesos a los nodos del graph
  ...
  void setTiledMap(RTSTiledMap *pMap) {
    m_pTiledMap = pMap;
  }			//Establecemos un puntero al mapa que será utilizado para todos los cálculos
  RTSTiledMap *getMapGrid() {
    return m_pTiledMap;
  }					//Regresa el puntero al mapa que se está utilizando para todos los cálculos

  void setStartPosition(const int32 x, const int32 y) {
    m_StartPos.x = x; m_StartPos.y = y;
  }	//Esta función se utiliza para establecer el punto inicial de búsqueda
  void setEndPosition(const int32 x, const int32 y) {
    m_EndPos.x = x; m_EndPos.y = y;
  }			//Esta función se utiliza para establecer el punto final de la búsqueda

  void getStartPosition(int32 & x, int32 & y) {
    x = m_StartPos.x; y = m_StartPos.y;
  }				//Función de ayuda para obtener el punto inicial de búsqueda
  void getEndPosition(int32 & x, int32 & y) {
    x = m_EndPos.x; y = m_EndPos.y;
  }						//Función de ayuda para obtener el punto final de búsqueda
